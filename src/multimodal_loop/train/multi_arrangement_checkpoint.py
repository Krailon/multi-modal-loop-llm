...
def save_multi_checkpoint(
    ...
    *,
    ...
    validate_multi_protocol(manifest, model.config, training, budget, smoke=smoke)
...
def load_multi_checkpoint(path, *, device="cpu"):
    """Load frozen weights on CPU/CUDA without restoring training randomness."""
    payload = torch.load(path, map_location="cpu", weights_only=True)
    if (
        not isinstance(payload, dict)
        or payload.get("kind") != "multi_arrangement_quartet_fit"
        ...
        raise ValueError("incompatible multi-arrangement checkpoint")
...
def validate_multi_protocol(manifest, model, training, budget, *, smoke=False):
    ...
    if not smoke and (
        manifest.source.sha256 != FIT_SHA256
        or tuple(manifest.derivation["training_arrangements"]) != TRAIN_NAMES
        ...
        raise ValueError("research settings disagree with fixed multi-arrangement protocol")

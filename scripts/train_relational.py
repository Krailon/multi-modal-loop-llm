"""Train fixed-depth relational color answers and evaluate held-out validation layouts."""
...
from multimodal_loop.data.relational_dataset import RelationalColorDataset, load_relational_manifest
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/relational_training"))
    ...
    try:
        ...
        if args.resume is not None:
            saved = load_relational_checkpoint(args.resume, device=device)
            ...
            manifest = load_relational_manifest(
                getattr(args, "manifest", Path("outputs/relational_shapes/manifest.json"))
            )
            model_overrides = {}
            ...
            config = ModelConfig(
                **{"vocab_size": 17, "image_size": manifest.config.image_size, **model_overrides}
            )
            validate_relational_config(config, manifest)
            training_overrides = {
                key: vars(args)[key]
    ...
    train_data = RelationalColorDataset(manifest, "train")
    validation_data = RelationalColorDataset(manifest, "validation")
    validation_loader = relational_loader(validation_data, model.config, training)
    ...
    for epoch in range(completed_epochs, completed_epochs + args.epochs):
        metrics = train_synthetic_epoch(
            model,
            optimizer,
            relational_loader(train_data, model.config, training, epoch=epoch),
